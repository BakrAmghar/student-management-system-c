...
int Modifier_GUI(char old_id[], char new_id[], char new_nom[], float new_moy) {
    if (strcmp(old_id, new_id) != 0) {
        if (RechercheID_GUI(new_id) != NULL) return 0;
    }
...
int compID(const void *a, const void *b) { return strcmp((*(etudiant**)a)->id, (*(etudiant**)b)->id); }
int compNom(const void *a, const void *b) { return strcmp((*(etudiant**)a)->nom, (*(etudiant**)b)->nom); }
